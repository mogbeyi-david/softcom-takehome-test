...
describe("User Resource", () => {
	...
	describe("Updating a user", () => {
		...
		it("should return 400 if the firstname, lastname or email is missing",
			async () => {
				const badPayload = {};
				const response = await request(server).
		...
		it("should return 400 if the newPassword and confirmNewPassword does not match",
			async () => {
				const testUser = await User.create({
		...
		it("should return 400 if the old password does not match the existing password",
			async () => {
				const testUser = await User.create({
					firstname: "test_first_name",
					lastname: "test_last_name",
		...
		it("should return 400 if the new password is not secure enough",
			async () => {
				const testUser = await User.create({
					firstname: "test_first_name",
					lastname: "test_last_name",
				...
				expect(response.body.message).
					toEqual("Password is not secure enough");
			});
		...
		it("should return 500 if the newPassword and confirmNewPassword is not passed",
			async () => {
				const testUser = await User.create({
					firstname: "test_first_name",
					lastname: "test_last_name",
